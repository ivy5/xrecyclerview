package com.ruijia.xrecyclerview;
...
import com.ruijia.xrecyclerview.adapter.RecyclerViewAdapter;
import com.ruijia.xrecyclerview.listener.OnItemClickListener;
import com.ruijia.xrecyclerview.view.RecyclerViewFooter;
import com.ruijia.xrecyclerview.view.RecyclerViewHeader;

public class XRecyclerView extends RecyclerView {
    private Scroller mScroller;
    private RecyclerViewAdapter lfAdapter;
    ...
    private XRecyclerViewListener mRecyclerViewListener;
    private boolean mPullLoad;
    private TextView mHeaderTimeView;
    private boolean isNoDateShow = false;
    private XRecyclerViewScrollChange scrollerListener;//滑动监听
...
//        observer=null;
    ...
    public void setAdapter(Adapter adapter) {
        ...
        lfAdapter = new RecyclerViewAdapter(getContext(), adapter);
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (isRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
            if (recyclerViewHeader.getVisiableHeight() > mHeaderViewHeight) {
                recyclerViewHeader.setState(RecyclerViewHeader.STATE_READY);
            } else {
                recyclerViewHeader.setState(RecyclerViewHeader.STATE_NORMAL);
            }
        }
    }
...
//        lfAdapter.notifyDataSetChanged();
        if (mPullRefreshing) {
            if (isSuccess) {
                recyclerViewHeader.setState(RecyclerViewHeader.STATE_SUCCESS);
            } else {
                recyclerViewHeader.setState(RecyclerViewHeader.STATE_FRESH_FAILT);
            }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (isLoadMore) {
            if (height > PULL_LOAD_MORE_DELTA) {
                recyclerViewFooter.setState(RecyclerViewFooter.STATE_READY);
                mPullLoading = true;
            } else {
                recyclerViewFooter.setState(RecyclerViewFooter.STATE_NORMAL);
                mPullLoading = false;
                mPullLoad = false;
            }
        }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (!mPullRefreshing && layoutManager.findFirstVisibleItemPosition() == 0) {
                    ...
                    if (isRefresh
                        ...
                        recyclerViewHeader.setState(RecyclerViewHeader.STATE_REFRESHING);
                ...
                if (isLoadMore && mPullLoading && layoutManager.findLastVisibleItemPosition() == lfAdapter.getItemCount() - 1
                    ...
                    recyclerViewFooter.setState(RecyclerViewFooter.STATE_LOADING);
                    mPullLoad = true;
                    startLoadMore();
                }
                resetHeaderHeight();
...
//            super.onItemRangeMoved(fromPosition + lfAdapter.getheaderViewCount(), toPosition + lfAdapter.getheaderViewCount(), itemCount);
    ...
    public void setLFRecyclerViewListener(XRecyclerViewListener l) {
    ...
    public interface XRecyclerViewScrollChange {
     ...
     */
    public void setScrollChangeListener(XRecyclerViewScrollChange listener) {
    ...
    public void setFootText(String text) {
        recyclerViewFooter.getHintView().setText(text);
    }
